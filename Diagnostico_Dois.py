...
print('------------------------ COVIDiag ------------------------')
print('\nBem-vindo à COVIDiag, ferramenta de diagnóstico de COVID-19.')
print('Digite S (Sim) ou N (Não) para cada sintoma apresentado a seguir:')
...
chikungunya = ['Febre', 'Dores intensas nas juntas', 'Pele e olhos avermelhados', 'Dores musculares intensas',
...
tuberculose = ['Febre', 'Sudorese noturna', 'Dificuldade para respirar', 'Perda de peso', 'Fadiga']

hipertensao = ['Dores no peito', 'Dor de cabeça', 'Tonturas', 'Zumbido no ouvido', 'Fraqueza', 'Visão embaçada', 'Sangramento nasal']

sintomas = ['Febre', 'Tosse', 'Dor de garganta', 'Perda de olfato', 'Dor de cabeça','Coriza','Dor no corpo',
            'Perda de paladar', 'Diarreia', 'Dor ao movimentar os olhos', 
            'Dor abdominal', 'Calafrios', 'Manchas vermelhas no corpo', 'Dificuldade para respirar',
            'Fadiga', 'Perda de apetite', 'Dores intensas nas juntas', 'Pele e olhos avermelhados',
            'Náuseas', 'Vômitos', 'Dores musculares intensas',  'Mal estar',
            'Sudorese noturna', 'Perda de peso', 'Chiado no peito',
            'Respiração rápida e curta', 'Desconforto torácico', 'Ansiedade', 'Dores no peito',
            'Tonturas', 'Zumbido no ouvido', 'Fraqueza', 'Visão embaçada', 'Sangramento nasal']

lista_sintomas = []


def consulta_sintomas(lista):  # função para iterar sobre a lista sintomas
    for i in lista:
        contador = True
        while contador:
            ...
            if positivo == 'S':  # caso ele tenha tal sintoma
                ...
                print(f'{positivo} é uma entrada inválida! Digite "S(sim) e N(não).')

consulta_sintomas(sintomas[0:5])  # inicia-se com os primeiros 5 sintomas da lista

for i in range(5, len(sintomas), 4):  # gerando os valores de i
    ...
    while contador:
        ...
        if resposta == 'S':  # caso a pessoa tenha mais sintomas outros 4 sintomas aparecrão pra ela na tela
            consulta_sintomas(sintomas[i:j])  # passa como parametro a sub lita que vai de i até j
            contador = False
        elif resposta == 'N':
            contador = False
            interromper = True #termina os laços iterativos sobre a lista de sintomas
        elif resposta != "S" and resposta != 'N':
            print(f'{resposta} é uma entrada invalida digite "S(sim) e N(não).')
    if interromper == True:
        break       
...
porcentagem = [porcentagem_asma, porcentagem_chikungunya, porcentagem_covid,
...
print('\nObrigado por utilizar o COVIDiag, ferramenta para o diagnóstico de COVID-19.')
